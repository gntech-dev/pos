                    ...
                    <div key={user.id} className="bg-gradient-to-br from-white to-indigo-50 border-2 border-indigo-100 rounded-lg p-4 hover:shadow-lg transition-all duration-200">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-base text-gray-800 truncate">{user.name}</h3>
...
function SecuritySettings() {
  ...
  const loadAuditLogs = useCallback(async () => {
    try {
      const response = await fetch('/api/audit?page=1&limit=20')
      if (response.ok) {
        const data = await response.json()
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <h3 className="font-semibold text-yellow-800 mb-2">⚠️ Alertas de Seguridad</h3>
            <ul className="text-sm text-yellow-700 space-y-1">
              <li>• Monitorea intentos de login fallidos</li>
              <li>• Revisa accesos desde IPs desconocidas</li>
